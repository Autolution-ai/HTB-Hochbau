  ...
  return (
    <section className="bg-anthracite py-24 text-white">
      <div className="mx-auto grid max-w-6xl gap-14 px-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <p className="section-label text-brand-light">Warum HTB Hochbau</p>
          ...
          <div className="mt-10 grid gap-8 sm:grid-cols-2">
            {reasons.map((reason, i) => (
              <Reveal key={reason.title} delay={i * 0.06}>
                <div>
                  <reason.icon size={26} className="text-brand-light" />
        ...
        <Reveal delay={0.1}>
          ...
          <div className="border border-white/15 p-6">
            ...
            <ul className="mt-4 space-y-3 text-sm">
              {contacts.map((c) => (
                <li key={c.role}>
                  <span className="text-brand-light">{c.role}</span>
                  <br />
            ...
            <ul className="mt-4 space-y-2 text-sm text-white/75">
              {certifications.map((cert) => (
                <li key={cert} className="flex items-start gap-2">
                  <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 bg-brand-light" />
                  {cert}
                </li>
              ))}
            </ul>
